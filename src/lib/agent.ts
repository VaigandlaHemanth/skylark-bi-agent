...
2. When the user names a board value exactly, filter on it. When they use a broader or informal word, PASS THEIR WORD THROUGH UNCHANGED and let query_board resolve it - do not translate it yourself. Asking for sector "energy" resolves to every energy-type sector; picking one of them for the engine silently drops the others and understates the answer. The result reports what it matched under "resolved_values" - repeat that to the user. Only call distinct_values if the resolution comes back empty.
